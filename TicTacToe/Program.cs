...
namespace TicTacToe
{
    //NOTE for future(?): it`s better to make smth to catch exceptions
    class Program
    {
        ...
        //we need "AppendLine" to to make our cells look nice, so Stringbuilder is a must
        static string GetPrintableState()
        {
            var sb = new StringBuilder();
            //drawing our board with numbers or symbols
            for (int i = 1; i <= 7; i += 3)
            {
                sb.AppendLine("     |     |     |")
                    .AppendLine(
